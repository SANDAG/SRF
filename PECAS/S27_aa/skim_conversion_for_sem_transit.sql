...
case when sum(f."flows")=0 then min(a.am_allpen_totalwait) else sum(a.am_allpen_totalwait * f.flows)/sum(f."flows") as am_allpen_totalwait,
case when sum(f."flows")=0 then min(a.am_allpen_totalwalk) else sum(a.am_allpen_totalwalk * f.flows)/sum(f."flows") as am_allpen_totalwalk,
case when sum(f."flows")=0 then min(a.am_allpen_xfers) else sum(a.am_allpen_xfers * f.flows)/sum(f."flows") as am_allpen_xfers
